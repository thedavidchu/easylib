...
static inline uint64_t _djb2_hash(void const *const buffer, size_t const size,
                                  uint64_t const seed) {
  uint64_t hash = seed;
  for (size_t i = 0; i < size; ++i) {
    hash = ((hash << 5) + hash) + ((uint8_t *)buffer)[i]; /* hash * 33 + c */
  }
  return hash;
}
...
static inline uint64_t EasyTable__hash(struct EasyTable const *const me) {
  ...
  for (size_t i = 0; i < me->length; ++i) {
    if (me->data[i].valid) {
      ...
      // NOTE This is probably really weak for a hash function but we want to
      // make it consistent even if the key-value pairs are scrambled.
      uint64_t obj_hash = _djb2_hash(&key_hash, sizeof(key_hash), val_hash);
      hash += obj_hash;
    }
  }
  return hash;
...
static inline uint64_t EasyList__hash(struct EasyList const *const me) {
  EASY_GUARD(me != NULL, "me should not be NULL");
  uint64_t hash =
...
static inline uint64_t EasyText__hash(struct EasyText const *const me) {
  EASY_GUARD(me != NULL, "me should not be NULL");
  uint64_t hash =
      _djb2_hash(&me->length, sizeof(me->length), DJB2_INITIAL_SEED);
...
static inline uint64_t EasyInteger__hash(struct EasyInteger const *const me) {
  EASY_GUARD(me != NULL, "me should not be NULL");
  uint64_t hash =
      _djb2_hash(&me->length, sizeof(me->length), DJB2_INITIAL_SEED);
  hash = _djb2_hash(me->data, me->length, hash);
...
static inline uint64_t EasyFraction__hash(struct EasyFraction const *const me) {
  EASY_GUARD(me != NULL, "me should not be NULL");
  uint64_t hash = 0;
...
static inline uint64_t EasyBoolean__hash(enum EasyBoolean const *const me) {
  EASY_GUARD(me != NULL, "me should not be NULL");
  uint64_t hash = _djb2_hash(me, sizeof(*me), DJB2_INITIAL_SEED);
  return hash;
}

static inline uint64_t EasyNothing__hash(EasyNothing const *const me) {
  EASY_GUARD(me != NULL, "me should not be NULL");
  uint64_t hash = _djb2_hash(me, sizeof(*me), DJB2_INITIAL_SEED);
  return hash;
}
...
#include "common/easyunused.h"

static bool test_hash_easy_table(void) {
  EASY_NOT_IMPLEMENTED();
  return true;
}

static bool test_hash_easy_list(void) {
  EASY_NOT_IMPLEMENTED();
  return true;
}

static bool test_hash_easy_text(void) {
...
static bool test_hash_easy_integer(void) {
...
static bool test_hash_easy_fraction(void) {
  EASY_NOT_IMPLEMENTED();
  return true;
}

static bool test_hash_easy_boolean(void) {
...
static bool test_hash_easy_nothing(void) {
